...
namespace MSD.Loop.Repository.Data
{
    public class CompanyAccessRoleRepository : BaseRepository, ICompanyAccessRoleRepository
    {
        ...
        {
            var query = @"SELECT * FROM CompanyAccessRoles 
                          WHERE CompanyId = @CompanyId";

            var results = Connection.Query<CompanyAccessRole>(query, 
                    param: new
                    {
                        CompanyId = id
        ...
        public CompanyAccessRole Create(CompanyAccessRole entity)
        {
            try
            {
                ...
                var query = @"INSERT INTO CompanyAccessRoles (AccessLevelId, CompanyId) 
                              VALUES(@AccessLevelId, @CompanyId); 
        ...
        {
            var query = @"DELETE FROM CompanyAccessRoles 
                          WHERE Id = @Id";
