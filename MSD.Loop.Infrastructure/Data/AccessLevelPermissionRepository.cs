...
namespace MSD.Loop.Infrastructure.Data
{
    public class AccessLevelPermissionRepository : BaseRepository, IAccessLevelPermissionRepository
    {
        ...
        public AccessLevelPermission Create(AccessLevelPermission entity)
        {
            try
            {
                var query = @"INSERT INTO AccessLevelPermissions (PermissionId, AccessLevelId) VALUES(@PermissionId, @AccessLevelId);  SELECT CAST(SCOPE_IDENTITY() as int)";
                var result = Connection.QuerySingle<int>(query, param: new { PermissionId = entity.Permission.Id, AccessLevelId = entity.AccessLevel.Id }, transaction: Transaction);
                entity.Id = result;
                return entity;
            }
        ...
        {
            var query = @"DELETE FROM AccessLevelPermissions WHERE Id = @Id";
        ...
        {
            var query = @"SELECT * FROM AccessLevelPermissions WHERE Id = @Id";
            var permission = Connection.QueryFirstOrDefault<AccessLevelPermission>(query, param: new { Id = id }, transaction: Transaction);
            if (permission == null)
            {
        ...
        {
            var query = @"UPDATE AccessLevelPermissions SET Name = @Name WHERE Id = @Id";
            var result = Connection.Execute(query,
                param: new { PermissionId = entity.Permission.Id, AccessLevelId = entity.AccessLevel.Id },
